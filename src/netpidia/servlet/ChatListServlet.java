...
import netpidia.ChatBean;
import netpidia.ChatMgr;



//@WebServlet("/netpidia/ChatListServlet")
public class ChatListServlet extends HttpServlet {
	...
	public String getToday() {
		...
		ChatMgr chat = new ChatMgr();
		ArrayList<ChatBean> chatList = chat.getChatList(new SimpleDateFormat("yyyy-MM-dd").format(new Date()));
	...
	public String getTen() {
		...
		result.append("{\"result\":[");
		ChatMgr chat = new ChatMgr();
		ArrayList<ChatBean> chatList = chat.getChatListByRecent(10);
		for(int i = 0; i < chatList.size(); i++) {
			result.append("[{\"value\": \"" + chatList.get(i).getChaName() + "\"},");
			result.append("{\"value\": \"" + chatList.get(i).getChatContent() + "\"},");
			result.append("{\"value\": \"" + chatList.get(i).getChatTime() + "\"}]");
	...
	public String getID(String chatID) {
		...
		result.append("{\"result\":[");
		ChatMgr chat = new ChatMgr();
		ArrayList<ChatBean> chatList = chat.getChatListByRecent(chatID);
		for(int i = 0; i < chatList.size(); i++) {
			result.append("[{\"value\": \"" + chatList.get(i).getChaName() + "\"},");
			result.append("{\"value\": \"" + chatList.get(i).getChatContent() + "\"},");
			result.append("{\"value\": \"" + chatList.get(i).getChatTime() + "\"}]");
